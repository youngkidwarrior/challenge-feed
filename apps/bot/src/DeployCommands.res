...
module Routes = {
  ...
  external applicationGuildCommands: (~clientId: string, ~guildId: string) => string =
...
let guildId = envConfig["discordGuildId"]

// @TODO: Shouldn't need to hardcode each command, instaed loop through files
let exampleCommand = Commands_Help.data->SlashCommandBuilder.toJSON

let commands = [exampleCommand]

let rest = Rest.make({"version": 9})->Rest.setToken(token)

let applicationGuildCommands = Routes.applicationGuildCommands(~clientId, ~guildId)

rest
->Rest.put(applicationGuildCommands, {"body": commands})
->then(() => Js.log("Successfully registered application commands.")->resolve)
->catch(e => {
  switch e {
